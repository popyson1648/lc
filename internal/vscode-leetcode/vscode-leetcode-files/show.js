...
const { getProblemDirPath } = require("./problemConfig");
...
exports.showSolution =
    exports.searchProblem =
        exports.showProblem =
            exports.pickOne =
                exports.previewProblem =
                    void 0;
...
function previewProblem(input, isSideMode = false) {
    ...
    return __awaiter(this, void 0, void 0, function* () {
        ...
        if (input instanceof vscode.Uri) {
            ...
            const cachedNode =
                explorerNodeManager_1.explorerNodeManager.getNodeById(id);
            if (!cachedNode) {
                vscode.window.showErrorMessage(
            ...
            node = input;
            const { isPremium } =
                (_a = globalState_1.globalState.getUserStatus()) !== null &&
                _a !== void 0
                    ? _a
                    : {};
            if (input.locked && !isPremium) {
                const url =
                    plugin_1.getLeetCodeEndpoint() === shared_1.Endpoint.LeetCode
                        ? shared_1.PREMIUM_URL_GLOBAL
                        : shared_1.PREMIUM_URL_CN;
                uiUtils_1.openUrl(url);
                return;
            }
        }
        const needTranslation = settingUtils.shouldUseEndpointTranslation();
        const descString = yield leetCodeExecutor_1.leetCodeExecutor.getDescription(
            node.id,
            needTranslation,
        );
        leetCodePreviewProvider_1.leetCodePreviewProvider.show(
            descString,
            node,
            isSideMode,
        );
...
function showSolution(input) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        try {
            const needTranslation = settingUtils.shouldUseEndpointTranslation();
            const solution = yield leetCodeExecutor_1.leetCodeExecutor.showSolution(
                problemInput,
                language,
                needTranslation,
            );
            leetCodeSolutionProvider_1.leetCodeSolutionProvider.show(
                unescapeJS(solution),
            );
        } catch (error) {
            leetCodeChannel_1.leetCodeChannel.appendLine(error.toString());
            yield uiUtils_1.promptForOpenOutputChannel(
                "Failed to fetch the top voted solution. Please open the output channel for details.",
                uiUtils_1.DialogType.error,
            );
        }
    });
}
        ...
        (() =>
            __awaiter(this, void 0, void 0, function* () {
                if (
                    ...
                    const choice = yield vscode.window.showInformationMessage(
                        `Would you like to set '${language}' as your default language?`,
                        uiUtils_1.DialogOptions.yes,
                        uiUtils_1.DialogOptions.no,
                        uiUtils_1.DialogOptions.never,
                    );
        ...
        try {
            ...
            const fileName = leetCodeConfig
                .get(
                    ...
                    problemUtils_1.genFileName(node, language),
                )
                .trim();
            ...
            if (finalPath) {
                ...
                if (!finalPath) {
                    leetCodeChannel_1.leetCodeChannel.appendLine(
                        "Showing problem canceled by user.",
                    );
                    return;
                }
            }
            ...
            const needTranslation = settingUtils.shouldUseEndpointTranslation();
            yield leetCodeExecutor_1.leetCodeExecutor.showProblem(
                node,
                language,
                finalPath,
...
function showDescriptionView(node) {
    return __awaiter(this, void 0, void 0, function* () {
        return previewProblem(
            node,
            vscode.workspace.getConfiguration("leetcode").get("enableSideMode", true),
        );
    });
}
function parseProblemsToPicks(p) {
    return __awaiter(this, void 0, void 0, function* () {
        return new Promise((resolve) =>
            __awaiter(this, void 0, void 0, function* () {
                const picks = (yield p).map((problem) =>
                    Object.assign(
                        {},
                        {
...
function resolveRelativePath(relativePath, node, selectedLanguage) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        const problemDirPath = getProblemDirPath();
        return relativePath.replace(/\$\{(.*?)\}/g, (_substring, ...args) => {
            ...
            switch (placeholder) {
                ...
                case "company":
                    return company;
                case "problem-dir-path":
                    return problemDirPath;
                default:
